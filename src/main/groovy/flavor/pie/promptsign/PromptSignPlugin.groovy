...
class PromptSignPlugin implements Plugin<Project> {
    ...
    void apply(Project target) {
        def extension = target.extensions.create('promptsign', PromptSignExtension)
        target.with {
            ...
            signArchives.doFirst {
                if (!extension.runOnBuild && gradle.taskGraph.hasTask(':build')) {
                    return
                }
        ...
        target.afterEvaluate {
            if (!extension.runOnBuild) {
                target.with {
                    signing {
                        required { !gradle.taskGraph.hasTask(':build') }
                    }
                }
            }
        }
